# -*- coding: utf-8 -*-
"""
QUANT_OS/BACKEND_LAYER/Data_Center/Engines/Trades/FTMO_DEMO_1.py
...
- Speichert inkrementell in:
    <QUANT_ROOT>/INFRASTRUCTURE_LAYER/Storage/1_Pipeline/Trades/live/account_<login>_<account_type>/closed_trades.db
- Polling im Loop
- Robuster gegen:
...
- Zusätzlich werden initiale SL/TP Werte gespeichert, sofern MT5 sie über historische Orders liefert.
- Zusätzlich wird ein schlanker Live-State offener Positionen gespeichert:
    open_positions_current.db / Tabelle open_positions_current
  Es wird nur eine aktuelle Zeile pro offener Position gehalten, keine Tick-Historie.
- Offene Positionen speichern Live-PnL, Best/Worst Floating PnL, Best/Worst Price, MFE/MAE Points.
- Beim Schließen werden vorhandene Live-Extremwerte in closed_trades übernommen.
- Es wird UPSERT verwendet statt INSERT OR IGNORE.

# ============================================================
# CODE_REGISTRY
# ============================================================
# script_id: trade_logger_demo_1513550384
# script_name: Trade Logger DEMO 1513550384
# owner: Leon Everts
# status: active
# layer: 1_Pipeline
# domain: Trades
# asset_type: Pipeline
# purpose: Logs MT5 closed trades and current open-position state into SQLite for demo account 1513550384.
# inputs:
#   - MetaTrader 5 terminal64.exe
#   - MT5 login/server/password from script defaults or environment variables
...
#   - data_pipeline
# notes:
#   - Output root is INFRASTRUCTURE_LAYER/Storage/1_Pipeline/Trades/live.
#   - Default MT5 credentials from the original script are preserved.
#   - Environment variables can override terminal path, login, password, and server.
# ============================================================
"""
...
CODE_REGISTRY: Dict[str, object] = {
    ...
    "reviewer": "ChatGPT",
    "created_date": "2026-06-01",
    "account_login_default": "1513550384",
    "account_type_expected": "DEMO",
    "mt5_server_default": "FTMO-Demo",
    "terminal_exe_default": r"C:\Users\Leon\Desktop\Terminals\MetaTrader 5 - Kopie - Kopie - Kopie (16) - Kopie - Kopie - Kopie - Kopie\terminal64.exe",
    "tags": [
        "mt5",
        "trades",
        "closed_trades",
...
POLL_SECONDS = int(os.getenv("TRADE_LOGGER_POLL_SECONDS", "60"))
POLL_OVERLAP_SECONDS = int(os.getenv("TRADE_LOGGER_POLL_OVERLAP_SECONDS", "300"))
INITIAL_LOOKBACK_DAYS = int(os.getenv("TRADE_LOGGER_INITIAL_LOOKBACK_DAYS", "365"))
POSITION_REBUILD_LOOKBACK_DAYS = int(os.getenv("TRADE_LOGGER_POSITION_REBUILD_LOOKBACK_DAYS", "365"))

DEBUG = os.getenv("TRADE_LOGGER_DEBUG", "1") == "1"
...
def main() -> None:
    ...
    log_info(f"DATA_CENTER_DIR = {DATA_CENTER_DIR.resolve()}")
